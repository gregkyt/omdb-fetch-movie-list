...
final class GetMovieSpec: XCTestCase {
    ...
    func test_execute_should_be_success() {
        ...
        _ = mockedRepository.when().call(
            withReturnValue:
                mockedRepository.getMovie(title: "")
        ).thenReturn(promise)
        
        let response = sut(repository: mockedRepository).execute(title: "")
    ...
    func sut(repository: MockedMovieRepository) -> GetMovieUseCaseImpl {
        return GetMovieUseCaseImpl(repository: repository)
    }
}
